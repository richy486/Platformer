...
//  }
  ...
  func cap(fallingVelocity velY: Double) -> Double {
    if velY > MAXVELY {
      return MAXVELY
    }
    return velY
  ...
  func collisionDetection(level: Level) -> Level {
    ...
    let targetPlayerPostition = Point(x: f.x + vel.x, y: f.y + vel.y)
    ...
    if slopeResult.collide {
      ...
      vel.y = Double(1) // What is this for?
      ...
      if slopeDir != .stationary {
        ...
        var s = IntPoint(x: Int(f.x + Double(size.width)/2 + vel.x), y: 0)
        if slopeDir.contains(.up) {
          potentialPosition.y = Double(slopeResult.collideTile.y * TILESIZE - size.height - 1)
          s.y = Int(potentialPosition.y) + size.height
        } else if slopeDir.contains(.down) {
          potentialPosition.y = Double((slopeResult.collideTile.y + 1) * TILESIZE - size.height - 1)
        ...
        if t.contains(.slope_right) {
          ...
          f.y = Double(yGround - inside - size.height - 1)
          
          return level
        } else if t.contains(.slope_left) {
          // ◿
          f.x += vel.x
          f.y = Double((collideTile.y+1)*TILESIZE - s.x%TILESIZE - size.height - 1)
          return level
        }
    ...
    if vel.x > 0.01 {
      ...
      let adjustedSize = Size(width: size.width, height: slopesBelow.right == nil ? size.height : heightSlope)
      
      var collide = false
      while f.x < targetPlayerPostition.x - COLLISION_GIVE && !collide {
        f.x = min(f.x + Double(TILESIZE), targetPlayerPostition.x)
      ...
      let adjustedSize = Size(width: size.width, height: slopesBelow.left == nil ? size.height : heightSlope)
      
      var collide = false
      while f.x > targetPlayerPostition.x + COLLISION_GIVE && !collide {
        f.x = max(f.x - Double(TILESIZE), targetPlayerPostition.x)
...
//      print("iPlayerL: \(iPlayerL), iPlayerC: \(iPlayerC), iPlayerR: \(iPlayerR)")
      ...
      if i.x + (size.width/2) < overlaptxl {
        alignedBlockX = txl
        unAlignedBlockX = txr
        unAlignedBlockFX = Double((txr << 5) - size.width) - COLLISION_GIVE
      } else {
        alignedBlockX = txr
        unAlignedBlockX = txl
        unAlignedBlockFX = Double((txl << 5) + TILESIZE) + COLLISION_GIVE
      }
      ...
      if vel.y < -0.01 {
        ...
        while f.y > targetPlayerPostition.y + COLLISION_GIVE && !collide {
          f.y = max(f.y - Double(TILESIZE), targetPlayerPostition.y)
        ...
        while f.y < targetPlayerPostition.y - COLLISION_GIVE && !collide {
          f.y = min(f.y + Double(TILESIZE), targetPlayerPostition.y)
